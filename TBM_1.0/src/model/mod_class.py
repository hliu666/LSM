...
# ------------------------------------------------------------------------------
    ...
    def dalecv2(self, p):
        ...
        nee_h, lst_h, an_h, re_h, brf_h, apar_h, par_h, sif_h = [], [], [], [], [], [], [], []
        An_daily = 0 
        ...
        for h in range(0, 24):
            xh = self.x*24+h
            ebal_pars, k_pars, brf, canopy_pars, hemi_brf, dif_brf = rtm_o(self.dC, xh, lai, hemi_dif_brf)
            #Ccu, Cch, Tcu, Tch, ecu, ech, APARu, APARh, Esolar, lst, Fc = Ebal_single(self.dC, xh, lai, ebal_pars, k_pars)
            Ccu, Cch, Tcu, Tch, ecu, ech, APARu, APARh, PAR, Esolar, lst, Fc = Ebal(self.dC, xh, lai, ebal_pars, k_pars)
            if (self.dC.tts[xh] < 75) and (lai > 0.5) and np.sum(Esolar[0][0:350])>0: 
                ...
                rcw_u,_,Anu = PhotoSynth(meteo_u)
                rcw_h,_,Anh = PhotoSynth(meteo_h)
                An = (Anu*Fc + Anh*(1-Fc))*lai
                sif = cal_canopy_sif(self.dC, xh, Esolar, canopy_pars, hemi_brf, dif_brf, hemi_dif_brf)            

            else:
                Rdu = -calc_resp(Rd25, Ear, Tcu+T2K)
                Rdh = -calc_resp(Rd25, Ear, Tch+T2K)
                An = (Rdu*Fc + Rdh*(1-Fc))*lai
                rcw_u, rcw_h = -9999.0, -9999.0
                sif = [0.0] * 6
            ...
            lst_h.append(lst)
            an_h.append(An)
            re_h.append((rcw_u+rcw_h)/2.0)
            brf_h.append(brf)
            apar_h.append(APARu + APARh)
            par_h.append(PAR)
            sif_h.append(sif)
        ...
        return out, nee_h, lst_h, an_h, re_h, brf_h, apar_h, par_h, refl_mds, sif_h
    ...
    def mod_list(self, pvals):
        ...
        nee_y, lst_y, An_y, Re_y, brf_y, apar_y, par_y, refls, sif_y = [], [], [], [], [], [], [], [], []
        self.x = self.startrun
        for t in xrange(self.endrun-self.startrun):
            print(t)
            mod_list[(t+1)], nee_d, lst_d, An_d, Re_d, brf_d, apar_d, par_d, refl_d, sif_d = self.dalecv2(mod_list[t])
            nee_y  += nee_d
            lst_y  += lst_d
            An_y   += An_d
            Re_y   += Re_d
        ...
        self.x -= self.endrun
        return mod_list, np.array(nee_y), np.array(sif_y)

...
def PhotoSynth_Jen(meteo):
    ...
    Vcmax_1 = RUB*kc              # [umol CO2 m-2 s-1] Max Rubisco activity 
    ...
    """
    kq = 40 # [umol e-1 umol sites-1 s-1] Cyt b6f kcat for PQH2
    ...
    A, Aj = Compute_A(Ci, Vcmax, Km, JP700_j, nl, nc, gtc, Gamma_star, Rd)
    ...
    phi1F_a_dark = (q1_a)*Kf/(Kd + Kf) + (1 - q1_a)*Kf/(Kn1 + Kd + Kf)

    eta2 = phi2F_a/phi2F_a_dark
    eta1 = phi1F_a/phi1F_a_dark
    ...
    """
    fqe2 = Fs_a2/Fo_a*0.007
    fqe1 = Fs_a1/Fo_a*0.007
...
def opt_Ci(x0, A_Pars, Ci_Pars):
    ...
    A,_ = Compute_A(x0, Vcmax_m, Km, JP700_mj, nl, nc, gtc, Gamma_star, Rd_m)
...
A1, A2 = [], []
for Q in range(0, 2000, 50):
    for T in range(10, 35, 1):
        meteo = [Q, Cs, T, eb]
        r1, c1, a1, eta2, eta1 = PhotoSynth_Jen(meteo)    
        r2, c2, a2 = PhotoSynth(meteo)   
        A1.append(eta1)
        A2.append(eta2)

import matplotlib.pyplot as plt
plt.plot(A1, label='eta1')
plt.plot(A2, label='eta2')

...
class DalecData:
    ...
    """
    def __init__(self, start_yr, end_yr, site, ci_flag, ob_str=None):
        ...
        self.p0 = 1.1e-5  # theta_min, cl to cs decomp      (1e-5 - 1e-2) day-1
        self.p1 = 0.5     # f_auto, fraction of GPP respired  (0.3 - 0.7)
        self.p2 = 0.15    # f_fol, frac GPP to foliage        (0.01 - 0.5)
        self.p3 = 0.457    # f_roo, frac GPP to fine roots    (0.01 - 0.5)
        self.p4 = 1.001  # clspan, leaf lifespan               (1.0001 - 5)
        self.p5 = 4.8e-5  # theta_woo, wood C turnover      (2.5e-5 - 1e-3) day-1
        self.p6 = 6.72e-3  # theta_roo, root C turnover rate(1e-4 - 1e-2) day-1
        self.p7 = 0.024  # theta_lit, litter C turnover     (1e-4 - 1e-2) day-1
        self.p8 = 2.4e-5  # theta_som, SOM C turnover       (1e-7 - 1e-3) day-1
        self.p9 = 0.0193  # Theta, temp dependence exp fact(0.018 - 0.08)
        self.p10 = 130.  # d_onset, clab release date       (1 - 365) (60,150)
        self.p11 = 0.48  # f_lab, frac GPP to clab           (0.01 - 0.5)
        self.p12 = 20.  # cronset, clab release period      (10 - 100)
        self.p13 = 300.  # d_fall, date of leaf fall        (1 - 365) (242,332)
        self.p14 = 35.  # crfall, leaf fall period          (10 - 100)
        self.p15 = 60  # Vcmax25
        ...
        self.Cab    = np.full((1, 365+366+365), 32.7454)  
        self.Car    = np.full((1, 365+366+365), 4.84134) 
        self.Cm     = np.full((1, 365+366+365), self.lma/10000.0)         
        self.Cbrown = np.full((1, 365+366+365), 0.000587915) #brown pigments concentration (unitless).
        self.Cw     = np.full((1, 365+366+365), 0.0122215)  #equivalent water thickness (g cm-2 or cm).
        self.Ant    = np.full((1, 365+366+365), 4.94719)  #Anthocianins concentration (mug cm-2). 
        ...
        self.Mf,  self.Mb  = sif_leaf(self.Cab[0,0], self.Cm[0,0], self.Kab, self.Kall[:,0])

        """ 
        Parameters of Canopy-level SIF  
        """ 
        ...
        self.sif_coff = 0.5 #partition photosystm I and II
       
        self.MbI_diag[240:451, 0:351] = self.Mf*self.sif_coff
        self.MfI_diag[240:451, 0:351] = self.Mb*self.sif_coff
        
        self.MbII_diag[240:451, 0:351] = self.Mf*(1-self.sif_coff)
        self.MfII_diag[240:451, 0:351] = self.Mb*(1-self.sif_coff)

        self.MI_diag   = self.wleaf_diag + self.MbI_diag  + self.MfI_diag
        self.MII_diag  = self.wleaf_diag + self.MbII_diag + self.MfII_diag
        self.MA_diag   = self.wleaf_diag
        
        self.MI_diag_q  = np.linalg.matrix_power(self.MI_diag, 2).astype(np.float32)
        self.MII_diag_q = np.linalg.matrix_power(self.MII_diag, 2).astype(np.float32)
        self.MA_diag_q  = np.linalg.matrix_power(self.MA_diag, 2).astype(np.float32)
        ...
        """
        Parameters of energy balance model
        """

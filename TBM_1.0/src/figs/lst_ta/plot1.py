...
for i in range(len(y_lists)):
    for j in range(len(x_lists)):
        x, y, z = apar, y_lists[i], x_lists[j]
        ...
        """
        if i == 1:
            p1 = ax[i,j].scatter(xobs, yobs*1000, c=cobs, marker='o', s=15, cmap=plt.cm.jet, alpha=0.8)
        else:
            p1 = ax[i,j].scatter(xobs, yobs, c=cobs, marker='o', s=15, cmap=plt.cm.jet, alpha=0.8)
...
#cbar.ax.set_title('APAR',fontsize=ftsize)
                   
fig.tight_layout()          
plot_path = "../../../figs/lst_ta/plot_lai.jpg"

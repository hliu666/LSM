...
from math import pi
from RTM_initial import volscatt, CIxy
...
def calc_ebal_canopy_pars(dC, x, lai, Ls, ebal_rtm_pars):
    rhos, taus = dC.leaf
...
def rtm_t(lai, L, i0, iD, wl, Tcu,Tch,Tsu,Tsh, dC, x, k_pars):
    ...
    [lidfa, lidfb, tts, tto, psi] = Angle_Pars
    ...
    [soil_spectrum1, soil_spectrum2] = Soil_Spectrums
    
    rg = rsoil * (
        psoil * soil_spectrum1 + (1.0 - psoil) * soil_spectrum2
    ) #soil reflectance
    rg_t = np.array([0.06]*(2162-2000))
    rg = np.concatenate([rg[:2000],rg_t])
    ...
    CIs, CIo, ko, sob, sof = dC.CIs[x], dC.CIo[x], dC.ko[x], dC.sob[x], dC.sof[x] 
    ...
    ki = cal_K(lidf, tts, tto, psi)
    i  = 1 - np.exp(-ki * lai * CIs)
    w  = 1 - emisVeg
    ...
    #TIRv = 0.001*integrate.simpson(TIRv,wl)
    #TIRs = 0.001*integrate.simpson(TIRs,wl)

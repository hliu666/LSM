...
def Ebal(d, p, x, lai, rtm_o_dict):
    ...
    ebal_sunsha_pars = calc_ebal_sunsha(d, x, lai)
    ebal_canopy_pars = calc_ebal_canopy_pars(d, x, Ls, rtm_o_dict)
    net_rads, Esolars = calc_netrad_pars(d, x, lai, SW, L, ebal_sunsha_pars, ebal_canopy_pars)
    while CONT:  # while energy balance does not close
        netrad_dict = calc_netrad(d, x, lai, T_Pars, net_rads, rtm_o_dict, ebal_sunsha_pars)
        Fc, Fs = netrad_dict['Fc'], netrad_dict['Fs']
        rad_Rnuc, rad_Rnhc = netrad_dict['rad_Rnuc'], netrad_dict['rad_Rnhc']
        rad_Rnus, rad_Rnhs = netrad_dict['rad_Rnus'], netrad_dict['rad_Rnhs']

        if (d.tts[x] < 75) and (lai > 0.5):
            # ----------------------canopy intercepted wator and soil moisture factor---------------------
            _, fwet, _, sf = calc_hy_f(d, p, x, lai, Ev, ET)

            APARu = max(netrad_dict['APARu'], 1e-16)
            APARh = max(netrad_dict['APARh'], 1e-16)
        ...
        u_star, R_x, R_s = resistance(p, lai, l_mo, d.wds[x])
        rac = (lai + 1) * R_x
        ras = (lai + 1) * R_s

        # ----------------------soil moisture factor---------------------
        beta_e = calc_beta_e(p.Soil, d.sm_top[x])
    ...
    if abs(Tcu - Ta) > 10 or np.isnan(Tcu):
        if x > 0:
            Tcu = Ta + (d.t_mean[x] - d.t_mean[x - 1])
        else:
            Tcu = Ta + 0.3
    if abs(Tch - Ta) > 10 or np.isnan(Tch):
        if x > 0:
            Tch = Ta + (d.t_mean[x] - d.t_mean[x - 1])
        else:
            Tch = Ta + 0.1
    if abs(Tsu - Ta) > 10 or np.isnan(Tsu):
        if x > 0:
            Tsu = Ta + (d.t_mean[x] - d.t_mean[x - 1])
        else:
            Tsu = Ta + 3.0
    if abs(Tsh - Ta) > 10 or np.isnan(Tsh):
        if x > 0:
            Tsh = Ta + (d.t_mean[x] - d.t_mean[x - 1])
        else:
            Tsh = Ta + 3.0

    LST = rtm_t(d, p, x, lai, L, rtm_o_dict['iD'], rtm_o_dict['kc'], T_Pars)
    ...
    out = {'Ccu': Ccu,
           ...
           'Ev': Ev,
           'ET': ET,
           'ERnuc': netrad_dict['ERnuc'],
           'ELnuc': netrad_dict['ELnuc'],
           'ERnhc': netrad_dict['ERnhc'],
           'ELnhc': netrad_dict['ELnhc']
           }

...
def PhotoSynth_Jen(meteo, p):
    ...
    Rd = calc_resp(p.Rd25, p.Ear, T)

    # Calculate temperature dependancies on Vcmax and Jmax
    Vcmax = sf * peaked_arrh(p.RUB, p.Eav, T, p.deltaSv, p.Hdv)

    # Calculate cytochrome b6f complex
    kq = p.kq0 * exp(p.Eaq / R * (1 / p.Tref - 1 / T))  # Cyt b6f kcat for PQH2, s-1
    Vqmax = p.CB6F * kq  # Max Cyt b6f activity, mol e- m-2 s-1
    ...
    A, Aj, Ac = Compute_A(Cm, Vcmax, Km, JP700_j, p.nl, p.nc, Gamma_star, Rd)

    # Calculate intercellular CO2 concentration   
    Ci = Cm - (A - Rd) * p.ppm2bar / p.gm

    gs = max(0.01, 1.6 * A * p.ppm2bar / (Cs - Ci))  # stomatal conductance
    rcw = (Rhoa / (Mair * 1E-3)) / gs  # stomatal resistance

    JP700_a, JP680_a = calc_ETR(p, Cm, Ac, JP700_j, Gamma_star)

    fqe2, fqe1 = calc_fluorescence_activation(p, Q, JP700_a, JP680_a, JP700_j, kq)

    return rcw, Ci / p.ppm2bar, A, fqe2, fqe1
...
def calc_fluorescence_activation(p, Q, JP700_a, JP680_a, JP700_j, kq):
    ...
    Fo_a = p.a2 * p.Kf / (p.Kp2 + p.Kd + p.Kf) * p.eps2 + p.a1 * p.Kf / (p.Kp1 + p.Kd + p.Kf) * p.eps1
    ...
    """
    fqe2 = Fs_a2 / Fo_a * p.eta
    fqe1 = Fs_a1 / Fo_a * p.eta
...
def opt_Cm(x0, A_Pars, Cm_Pars):
    [Vcmax_m, Km, JP700_mj, nl, nc, Gamma_star, Rd_m] = A_Pars
    A, _, _ = Compute_A(x0, Vcmax_m, Km, JP700_mj, nl, nc, Gamma_star, Rd_m)
...
def Compute_A(Cm, Vcmax, Km, Je, nl, nc, Gamma_star, Rd):
    """
    Parameters
    ----------
    ...
    Aj = Je / 5.0 * ((Cm - Gamma_star) / (2 * Gamma_star + Cm))

    A = sel_root(theta_hyperbol, -(Ac + Aj), Ac * Aj, -1)

...
def run_model(pars):      
    ...
    m = mc.DalecModel(d)
    model_output, nee_y = m.mod_list(d.xb)
    nee_y = nee_y[0:26160]
    ...
    #lai_obs = np.full(365+366+365, 3.8)  
    #lar_err = np.full(365+366+365, 0.3)  
    
    lai_obs = np.full(1091, 3.8)  
    lar_err = np.full(1091, 0.1)  
    
    err = ((lai_obs-lai)/lar_err)**2
    cost_value += np.mean(err)*10
    ...
    if np.isnan(cost_value):
        ...
        #EDC2:NEE
        nee = np.array(d.vrfy_data['nee_gf'][0:26160])
        par = np.array(d.flux_data['PAR_up'][0:26160])

...
for mean, err, ylabel, ax in zip([mean1, mean2, mean3, mean4, mean5], [err1, err2, err3, err4, err5], ['NEE', 'LAI', 'LST', 'red', 'nir'], [axs[0], axs[1], axs[2], axs[3], axs[4]]):
    ...
    x_labels =  ["p1",  "p2",  "p3",  "p4",  "p5",  "p6",\
                 "p15", "p7",  "p9",  "p10", "p21", "p11",\
                 "p12", "p13", "p14", "p16", "p17", "p8",\
                 "p19", "p18", "p20", "p29", "p28", "p27",\
                 "p30", "p22", "p23", "p25", "p24", "p26"]

    rex_labels =  ["p1",  "p2",  "p3",  "p4",  "p5",  "p6",\
                   "p7",  "p8",  "p9",  "p10", "p11", "p12",\
                   "p13", "p14", "p15", "p16", "p17", "p18",\
                   "p19", "p20", "p21", "p22", "p23", "p24",\
                   "p25", "p26", "p27", "p28", "p29",  "p30"]
     
    name_labels = ["Clab", "Cfol", "Croo", "Cwoo", "Clit", "Csom",\
                   "fauto", "flab", "ffol", "froo", "Θwoo", "Θroo", "Θlit",\
                   "Θsom", "Θmin", "Θ", "donset", "dfall", "cronset","crfall",\
                    "clspan", "Cab", "Car", "Cw", "Cbrown", "Ant",\
                    "LIDFa", "CI", "Vcmax25", "m"]   
    df_mean = pd.DataFrame(mean, columns = x_labels)
    df_err  = pd.DataFrame(err,  columns = x_labels)

    df_mean = df_mean[rex_labels] 
    df_err  = df_err[rex_labels] 
    
    mean = np.array(df_mean)
    err  = np.array(df_err)
    
    x = np.arange(int(len(rex_labels)))
    ...
    R1 = ax.bar(x + 0 * bar_width,   mean[0,:], bar_width, yerr = err[0,:],  error_kw = {'ecolor' : '0.2', 'elinewidth':elinewidth, 'capsize' :capsize, 'capthick' :capthick}, color="white", edgecolor=S1_Color, label = "First order index", align="center", alpha=1)
    R2 = ax.bar(x + 1.28 * bar_width, mean[1,:], bar_width, yerr = err[1,:],  error_kw = {'ecolor' : '0.2', 'elinewidth':elinewidth, 'capsize' :capsize, 'capthick' :capthick}, color="white", edgecolor=ST_Color,  label = "Total order index", align="center", alpha=1)
    ...
    ax.set_title(ylabel, fontsize = ftsize*1.5, family=ftfamily)
    if ax == axs[4]:
        ax.set_xlabel('Parameters', fontsize = ftsize*1.5, family=ftfamily)
    
    ax.set_xticks(x + 1 * bar_width)
    ax.set_xticklabels(name_labels, fontsize = ftsize*1.5, family = ftfamily, rotation=45)
    if ax == axs[0]:
    ...
    #labels = ["First order index", "Total order index"] 
    labels = ["Main effect", "Total effect"] 
fig.tight_layout() 

...
def calc_netrad_pars(wl, lai, SW, L, ebal_sunsha_pars, ebal_canopy_pars):
    ...
    return [ERnuc, ERnhc, ERnus, ERnhs, APARu, APARh, PAR], [Esun_, Esky_]
 
def cal_reflectance(pars):
    [Cab, Car, Cm, Cbrown, Cw, lai, sw, Ta, sza] = pars
    ...
    [fsun, fsha, _] = ebal_sunsha_pars 
    [_, _, _, _, APARu, APARh, PAR] = net_rads
    
    meteo_u = [APARu/(fsun*lai), Ccu, Tcu, ecu]
    meteo_h = [APARh/(fsha*lai), Cch, Tch, ech]
    ...
    return gpp/PAR, gpp/APAR, gpp, PAR, APAR, brf[0:2006]
...
problem = {
    "num_vars": 9, 
    "names": ['Cab','Car','Cm','Cbrown','Cw','lai', 'sw', "Ta", "sza"], 
    "bounds": [[0, 100], [0, 10], [0, 0.015], [0, 1.0], [0, 0.01], [0, 6], [0.0, 1200], [0.0, 40.0], [0.0, 75.0]]
}
...
for pars in param_values:
    lue1, lue2, gpp, par, apar, brf = cal_reflectance(pars)
    ...
    if gpp > 0:
        ...
        out_2.append(par)
        out_3.append(apar)
        out_4.append(apar/par)
    
import joblib 
joblib.dump(out_x,  "../../figs/plsr/out_x.pkl")
joblib.dump(out_y1,  "../../figs/plsr/out_y1.pkl")
joblib.dump(out_y2,  "../../figs/plsr/out_y2.pkl")

joblib.dump(out_1,  "../../figs/plsr/out_gpp.pkl")
joblib.dump(out_2,  "../../figs/plsr/out_par.pkl")
joblib.dump(out_3,  "../../figs/plsr/out_apar.pkl")
joblib.dump(out_4,  "../../figs/plsr/out_fpar.pkl")

...
class Data:
    ...
    def __init__(self, pars, hourly_data_paths, daily_data_path, spectral_data_paths, fields):
        ...
        [carbon_pool_fields, RTM_optical_fields, Energy_balance_fields, biochemistry_fields, RTM_SIF_fields] = fields
        carbon_pool_df = self.create_pars(len(daily_df), carbon_pool_fields)
        RTM_optical_df = self.create_pars(len(hourly_df), RTM_optical_fields[:-self.spectral_length])
        spec_optical_df = self.hyperspectral_pars(len(hourly_df))
        Energy_balance_df = self.create_pars(len(hourly_df), Energy_balance_fields)
        ...
        self.hourly_df = pd.concat([hourly_df, spectral_df, RTM_optical_df, Energy_balance_df, biochemistry_df, RTM_SIF_df, spec_optical_df], axis=1)

...
def calc_ebal_canopy_pars(d, x, Ls, rtm_o_dict):
    rho, tau = d.leaf[0][:, x % 365], d.leaf[1][:, x % 365]
    w = rho + tau  # leaf single scattering albedo

    [t1, t3, t4, t5, t12, t16] = d.atmoMs

    rsd, rdd, rs, A_tot, A_dif, i0 = rtm_o_dict['R'], rtm_o_dict['R_dif'], rtm_o_dict['Rs'], rtm_o_dict['A_tot'], rtm_o_dict['A_dif'],rtm_o_dict['i0']
...
def calc_netrad_pars(d, x, lai, SW, L, ebal_sunsha_pars, ebal_canopy_pars):
    ...
    return [ERnuc, ERnhc, ERnus, ERnhs, APARu, APARh, PAR], [Esun_, Esky_, fEsuno, fEskyo]


def calc_netrad(d, x, lai, T_Pars, net_rads, rtm_o_dict, ebal_sunsha_pars):
    ...
    ERnuc, ERnhc, ERnus, ERnhs, APARu, APARh, PAR = net_rads
...
def rtm_t(dC, p, x, lai, L, iD, kc, T_Pars):
    """
    ...
    """
    i = max(1 - np.exp(-kc * lai * CIs), 1E-5)

...
for mean, err, ylabel, ax in zip([mean1, mean2, mean3, mean4, mean5], [err1, err2, err3, err4, err5],
                                 ['NEE', 'LAI', 'LST', 'red', 'nir'], [axs[0], axs[1], axs[2], axs[3], axs[4]]):
    ...
    R1 = ax.errorbar(x, mean[0, :], yerr=err[0, :], color=S1_Color, marker='o', markerfacecolor='white',
                     markersize=ftsize / 3, linestyle="none", transform=trans1)
    R2 = ax.errorbar(x, mean[1, :], yerr=err[1, :], color=ST_Color, marker='^', markerfacecolor='white',
                     markersize=ftsize / 2, linestyle="none", transform=trans2)

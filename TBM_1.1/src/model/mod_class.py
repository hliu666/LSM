...
# ------------------------------------------------------------------------------
    ...
    def dalecv2(self, p):
        ...
        return out, nee_h, an_h, sif_sun, sif_shade
    ...
    def mod_list(self, pvals):
        ...
        nee_y, An_y, sif_u_y, sif_h_y = [], [], [], []
        self.x = self.startrun
        for t in xrange(self.endrun-self.startrun):
            print(t)
            mod_list[(t+1)], nee_d, An_d, sif_u_d, sif_h_d = self.dalecv2(mod_list[t])
            nee_y  += nee_d
            An_y   += An_d
            sif_u_y   += sif_u_d
            sif_h_y  += sif_h_d
        ...
        self.x -= self.endrun
        return mod_list, np.array(nee_y), np.array(sif_u_y), np.array(sif_h_y)

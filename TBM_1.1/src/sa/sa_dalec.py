...
Y1 = np.full((sample.shape[0]), -9999.0)
Y2 = np.full((sample.shape[0]), -9999.0)
Y3 = np.full((sample.shape[0]), -9999.0)
Y4 = np.full((sample.shape[0]), -9999.0)
Y5 = np.full((sample.shape[0]), -9999.0)
Y6 = np.full((sample.shape[0]), -9999.0)
...
for i in range(len(Y1)):
    ...
    if os.path.exists(os.path.join(path, "nee_ci1_HARV_Dalec_{0}.pkl".format(i))) and  \
        ...
        #if s1 is not None:
        #s1d = np.clip(s1, -40, 15) #np.nanmean(s1.reshape(24,1095), axis=1)#[np.nanmean(s1[x: x+24]) for x in range(0, len(s1), 24)]
        #s2d = np.clip(s2[:,-2], 0, 7)
        #s3d = np.clip(s3, -5, 25) #np.nanmean(s3.reshape(24,1095), axis=1)
        #s4d = np.clip(s4[:, 0], 0, 1)
        #s5d = np.clip(s4[:, 1], 0, 1)
        ...
        Y1[i] = np.nanmean(s1d) #NEE
        Y2[i] = np.nanmean(s2d)   #LAI
        Y3[i] = np.nanmean(s3d)  #LST
        Y4[i] = np.nanmean(s4d)   #red
        Y5[i] = np.nanmean(s5d)   #nir
...
Y1[Y1==-9999] = Y1[Y1 != -9999].mean()
Y2[Y2==-9999] = Y2[Y2 != -9999].mean()
Y3[Y3==-9999] = Y3[Y3 != -9999].mean()
Y4[Y4==-9999] = Y4[Y4 != -9999].mean()
Y5[Y5==-9999] = Y5[Y5 != -9999].mean()

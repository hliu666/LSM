    ...
    def tbm(self, pd, ph, ps):
        ...
        for h in range(0, 24):
            ...
            if (self.d.tts[xh] < 75) and np.sum(Ebal_dict['Esolars'][0][0:350]) > 0 and (lai > 0.5):
                ...
                self.d.w_can[xh + 1], fwet, self.d.sm_top[xh + 1], sf = calc_hy_f(self.d, self.p, xh, lai,
                                                                                  Ebal_dict['Ev'],
                                                                                  Ebal_dict['ET'])
                # ----------------------two leaf model---------------------
                ...
                # ----------------------canopy intercepted wator and soil moisture factor---------------------
                self.d.w_can[xh + 1], self.d.sm_top[xh + 1] = self.d.w_can[xh], self.d.sm_top[xh]
                ...
                fqe2u, fqe1u, fqe2h, fqe1h = 0.0, 0.0, 0.0, 0.0
            ...
            ph_out[h] = [nee, An, rtm_o_dict['fPAR'], sif_u[2], sif_h[2], \
                         Ebal_dict['LST'], fqe2u, fqe1u, fqe2h, fqe1h, \
                         Ebal_dict['ERnuc'], Ebal_dict['ELnuc'], Ebal_dict['ERnhc'], Ebal_dict['ELnhc'], \
                         Ebal_dict['Tcu'], Ebal_dict['Tch'], Ebal_dict['Tsu'], Ebal_dict['Tsh'], \
                         self.d.tts[xh], self.d.tto[xh], self.d.psi[xh]]

            ps_out[h] = rtm_o_dict['BRF'][0:2101]
        ...
        return pd_out, ph_out, ps_out

    def mod_list(self, output_dim1, output_dim2, output_dim3):
        ...
        mod_list_spectral = np.full((self.endrun - self.startrun + 1, 24, output_dim3), 0.0)

        self.x = self.startrun
        for t in xrange(self.endrun - self.startrun):
            mod_list_daily[(t + 1)], mod_list_hourly[(t + 1)], mod_list_spectral[(t + 1)] = self.tbm(mod_list_daily[t], mod_list_hourly[t], mod_list_spectral[t])
            self.x += 1

        self.x -= self.endrun
        return mod_list_daily, mod_list_hourly, mod_list_spectral

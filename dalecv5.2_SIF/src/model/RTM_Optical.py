...
def rtm_o(dC, x, lai, hemi_dif_brf):
    ...
    Tup_difo = tv+iD*w*rho_obs/(1-p*w)
    Rdn_dif  = iD*w*rho_dif_hemi/(1-p*w)
    
    BRF_difv = sob_vsla_dif*rho+sof_vsla_dif*tau+iD*w**2*p*rho_obs/(1-p*w)          
    BRF_difs = kg_dif*rg    
    ...
    SIF 
    """
    hemi_pars = [sob_vsla,          sof_vsla,          kgd]
    dif_pars  = [sob_vsla_dif,      sof_vsla_dif,      kg_dif] 
    canopy_pars = [i0, iD, p, rho_obs, rho_hemi, tv, kc, kg]
    
    return ebal_pars, k_pars, hemi_pars, dif_pars, canopy_pars

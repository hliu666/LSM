...
from PhotoSynth import PhotoSynth, Respiration
from SIF import cal_canopy_sif
...
# ------------------------------------------------------------------------------
    ...
    def dalecv2(self, p):
        ...
        nee_h, lst_h, sif_h = [], [], []
        An_daily = 0 
        photo_pars   = [self.dC.Vcmax25, self.dC.BallBerrySlope]
        hemi_dif_brf = BRF_hemi_dif_func(self.dC.hemi_dif_pars, lai) 
        for h in range(0, 24):
            xh = self.x*24+h
            ebal_pars, k_pars, hemi_brf, dif_brf, canopy_pars = rtm_o(self.dC, xh, lai, hemi_dif_brf)
            #Ccu, Cch, Tcu, Tch, ecu, ech, APARu, APARh, Esolars, lst, Fc = Ebal_single(self.dC, xh, lai, ebal_pars, k_pars)
            Ccu, Cch, Tcu, Tch, ecu, ech, APARu, APARh, Esolars, lst, Fc = Ebal(self.dC, xh, lai, ebal_pars, k_pars)
            if (self.dC.tts[xh] < 90) and (lai > 0.5): 
                #print(xh)
                #----------------------two leaf model---------------------
                """
                APARu_leaf, APARh_leaf = APARu/lai, APARh/lai
                meteo_u = [APARu_leaf, Ccu, Tcu, ecu, self.dC.o, self.dC.p]
                meteo_h = [APARh_leaf, Cch, Tch, ech, self.dC.o, self.dC.p]
                _,_,Anu = PhotoSynth(meteo_u, photo_pars)
                _,_,Anh = PhotoSynth(meteo_h, photo_pars)
                An = Anu*lai + Anh*lai
                """
                meteo_u = [APARu, Ccu, Tcu, ecu, self.dC.o, self.dC.p]
                meteo_h = [APARh, Cch, Tch, ech, self.dC.o, self.dC.p]
                _,_,Anu = PhotoSynth(meteo_u, photo_pars)
                _,_,Anh = PhotoSynth(meteo_h, photo_pars)
                An = Anu + Anh
                sif = cal_canopy_sif(self.dC, xh, lai, Esolars, canopy_pars, hemi_brf, dif_brf, hemi_dif_brf)            
            else:
                APARu, APARh = 0, 0
                meteo_u = [APARu, Ccu, Tcu, ecu, self.dC.o, self.dC.p]
                meteo_h = [APARh, Cch, Tch, ech, self.dC.o, self.dC.p]
                Rdu = Respiration(meteo_u, photo_pars)
                Rdh = Respiration(meteo_h, photo_pars)
    ...
    def mod_list(self, pvals):
        ...
        for t in xrange(self.endrun-self.startrun):
            print(t)
            mod_list[(t+1)], nee_d, lst_d, sif_d = self.dalecv2(mod_list[t])
            nee_y  += nee_d
            lst_y  += lst_d
            sif_y  += sif_d
        ...
        self.x -= self.endrun
        return mod_list, np.array(nee_y), np.array(lst_y), np.array(sif_y)

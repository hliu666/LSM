...
class Dataloader:
    def __init__(self, dC, v, p):
        ...
        carp_X_train, carp_y_train, carp_X_valid, carp_y_valid, carp_X_test, carp_y_test, self.carp_label_scaler = self.dataLoader_daily(
            dC.daily_df,
            p.lookback_periods,
            p.test_portion,
            v.y_carp_vars,
        ...
        rtmo_X_train, rtmo_y_train, rtmo_X_valid, rtmo_y_valid, rtmo_X_test, rtmo_y_test, self.rtmo_label_scaler = self.dataLoader_hourly(
            dC.hourly_df,
            p.lookback_periods,
            p.test_portion,
            v.y_rtmo_vars,
        ...
        enba_X_train, enba_y_train, enba_X_valid, enba_y_valid, enba_X_test, enba_y_test, self.enba_label_scaler = self.dataLoader_hourly(
            dC.hourly_df,
            p.lookback_periods,
            p.test_portion,
            v.y_enba_vars,
        ...
        bicm_X_train, bicm_y_train, bicm_X_valid, bicm_y_valid, bicm_X_test, bicm_y_test, self.bicm_label_scaler = self.dataLoader_hourly(
            dC.hourly_df,
            p.lookback_periods,
            p.test_portion,
            v.y_bicm_vars,
        ...
        rtms_X_train, rtms_y_train, rtms_X_valid, rtms_y_valid, rtms_X_test, rtms_y_test, self.rtms_label_scaler = self.dataLoader_hourly(
            dC.hourly_df,
            p.lookback_periods,
            p.test_portion,
            v.y_rtms_vars,
        ...
        carp_valid_data = TensorDataset(torch.from_numpy(carp_X_valid), torch.from_numpy(carp_y_valid))
        rtmo_valid_data = TensorDataset(torch.from_numpy(rtmo_X_valid), torch.from_numpy(rtmo_y_valid))
        enba_valid_data = TensorDataset(torch.from_numpy(enba_X_valid), torch.from_numpy(enba_y_valid))
        bicm_valid_data = TensorDataset(torch.from_numpy(bicm_X_valid), torch.from_numpy(bicm_y_valid))
        rtms_valid_data = TensorDataset(torch.from_numpy(rtms_X_valid), torch.from_numpy(rtms_y_valid))
        ...
        self.carp_valid_loader = DataLoader(carp_valid_data, shuffle=True, batch_size=p.batch_size_daily, drop_last=True)
        self.rtmo_valid_loader = DataLoader(rtmo_valid_data, shuffle=True, batch_size=p.batch_size_hourly, drop_last=True)
        self.enba_valid_loader = DataLoader(enba_valid_data, shuffle=True, batch_size=p.batch_size_hourly, drop_last=True)
        self.bicm_valid_loader = DataLoader(bicm_valid_data, shuffle=True, batch_size=p.batch_size_hourly, drop_last=True)
        self.rtms_valid_loader = DataLoader(rtms_valid_data, shuffle=True, batch_size=p.batch_size_hourly, drop_last=True)

        self.carp_test_loader = DataLoader(carp_test_data, shuffle=False, batch_size=p.batch_size_daily, drop_last=True)
        self.rtmo_test_loader = DataLoader(rtmo_test_data, shuffle=False, batch_size=p.batch_size_hourly, drop_last=True)
        self.enba_test_loader = DataLoader(enba_test_data, shuffle=False, batch_size=p.batch_size_hourly, drop_last=True)
        self.bicm_test_loader = DataLoader(bicm_test_data, shuffle=False, batch_size=p.batch_size_hourly, drop_last=True)
        self.rtms_test_loader = DataLoader(rtms_test_data, shuffle=False, batch_size=p.batch_size_hourly, drop_last=True)
    ...
    def dataLoader_daily(self, df, lookback_daily, test_portion, y_field, x_field_vars, x_field_pars):
        ...
        train_end = int(total_len * (1 - 2 * test_portion))
        valid_end = int(total_len * (1 - test_portion))

        X_train, y_train = inputs[:train_end], labels[:train_end]
        X_valid, y_valid = inputs[train_end:valid_end], labels[train_end:valid_end]
        X_test, y_test = inputs[valid_end:], labels[valid_end:]

        return X_train, y_train, X_valid, y_valid, X_test, y_test, label_scaler

...
def rtm_o_mds(dC, x, lai):
    ...
    ax.plot(BRF[220:271],color='black',linestyle='solid', label='BRF')
    """
    ...
    sur_refl_b06 = float(np.nanmean(BRF[1218:1252].flatten()*dC.rsr_sw1.flatten())/np.nansum(dC.rsr_sw1.flatten()))
    sur_refl_b07 = float(np.nanmean(BRF[1705:1755].flatten()*dC.rsr_sw2.flatten())/np.nansum(dC.rsr_sw2.flatten())) 

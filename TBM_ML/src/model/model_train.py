...
def train(dL, batch_size, n_layers_list, learn_rate, hidden_dim_list, EPOCHS):
    [carp_hidden_dim, rtmo_hidden_dim, enba_hidden_dim, rtmt_hidden_dim, bicm_hidden_dim, rtms_hidden_dim] = hidden_dim_list
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for (carp_X, carp_y, rtmo_X, rtmo_y, enba_X, enba_y, rtmt_X, rtmt_y, bicm_X, bicm_y, rtms_X, rtms_y) in dL.train_loader:
            ...
            rtmt_loss = criterion(rtmt_out, rtmt_y.float())
            bicm_loss = criterion(bicm_out, bicm_y.float())
            rtms_loss = criterion(rtms_out, rtms_y.float())

            loss = carp_loss + rtmo_loss + enba_loss + rtmt_loss + bicm_loss + rtms_loss
            loss.backward()
            optimizer.step()
            avg_loss += loss.item()
        ...
        for (carp_X, carp_y, rtmo_X, rtmo_y, enba_X, enba_y, rtmt_X, rtmt_y, bicm_X, bicm_y, rtms_X, rtms_y) in dL.test_loader:
            carp_h = model.init_hidden(batch_size, carp_hidden_dim, carp_n_layers)
            rtmo_h = model.init_hidden(batch_size, rtmo_hidden_dim, rtmo_n_layers)
            enba_h = model.init_hidden(batch_size, enba_hidden_dim, enba_n_layers)
            rtmt_h = model.init_hidden(batch_size, rtmt_hidden_dim, rtmt_n_layers)

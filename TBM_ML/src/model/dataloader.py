...
class Dataloader:
    def __init__(self, dC, x_vars, x_pars, y_vars, lookback_daily, lookback_hourly, batch_size, test_portion):

        [x_carp_vars, x_rtmo_vars, x_enba_vars, x_rtmt_vars, x_bicm_vars, x_rtms_vars] = x_vars
        [x_carp_pars, x_rtmo_pars, x_enba_pars, x_rtmt_pars, x_bicm_pars, x_rtms_pars] = x_pars
        [y_carp_vars, y_rtmo_vars, y_enba_vars, y_rtmt_vars, y_bicm_vars, y_rtms_vars] = y_vars
        ...
        rtmt_X_train, rtmt_y_train, rtmt_X_test, rtmt_y_test, self.rtmt_label_scaler = self.dataLoader_hourly(
            dC.hourly_df,
            lookback_daily,
            lookback_hourly,
            test_portion,
        ...
        train_data = TensorDataset(torch.from_numpy(carp_X_train), torch.from_numpy(carp_y_train),
                                   ...
                                   torch.from_numpy(rtmt_X_train), torch.from_numpy(rtmt_y_train),
        ...
        test_data = TensorDataset(torch.from_numpy(carp_X_test), torch.from_numpy(carp_y_test),
                                  ...
                                  torch.from_numpy(rtmt_X_test), torch.from_numpy(rtmt_y_test),
                                  torch.from_numpy(bicm_X_test), torch.from_numpy(bicm_y_test),
                                  torch.from_numpy(rtms_X_test), torch.from_numpy(rtms_y_test))

        self.label_scalers = [self.carp_label_scaler, self.rtmo_label_scaler, self.enba_label_scaler,
                              self.rtmt_label_scaler, self.bicm_label_scaler, self.rtms_label_scaler]
        ...
        self.rtmt_input_dim = len(x_rtmt_pars+x_rtmt_vars)
        ...
        self.rtmt_output_dim = len(y_rtmt_vars)
        self.bicm_output_dim = len(y_bicm_vars)
        self.rtms_output_dim = len(y_rtms_vars)

        self.input_dim_list = [self.carp_input_dim, self.rtmo_input_dim, self.enba_input_dim, self.rtmt_input_dim, self.bicm_input_dim, self.rtms_input_dim]
        self.output_dim_list = [self.carp_output_dim, self.rtmo_output_dim, self.enba_output_dim, self.rtmt_output_dim, self.bicm_output_dim, self.rtms_output_dim]
    ...
    def dataLoader_hourly(self, df, lookback_daily, lookback_hourly, test_portion, y_field, x_field_vars, x_field_pars):
        df = df[y_field + x_field_vars + x_field_pars]
        ...
        # Define lookback period and split inputs/labels
        inputs = np.zeros((len(daily_hourly_data) - lookback_daily, lookback_daily, lookback_hourly,
                           len(x_field_vars + x_field_pars)))
        labels = np.zeros((len(daily_hourly_data) - lookback_daily, lookback_hourly, len(y_field)))

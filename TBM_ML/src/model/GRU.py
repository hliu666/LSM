...
class GRUModel(nn.Module):
    def __init__(self, input_dim_list, hidden_dim_list, output_dim_list, n_layers_list):
        ...
        [carp_input_dim, rtmo_input_dim, enba_input_dim, rtmt_input_dim, bicm_input_dim,
         rtms_input_dim] = input_dim_list
        ...
        self.rtmt_hidden_dim = rtmt_hidden_dim
        ...
        self.rtmt_n_layers = rtmt_n_layers
        ...
        self.rtmt_gru_cells = nn.ModuleList()
        for i in range(rtmt_n_layers):
            if i == 0:
                input_dim = rtmt_input_dim
            else:
                input_dim = rtmt_hidden_dim
            self.rtmt_gru_cells.append(nn.GRUCell(input_dim, rtmt_hidden_dim))
        ...
        [carp_output_dim, rtmo_output_dim, enba_output_dim, rtmt_output_dim, bicm_output_dim,
         rtms_output_dim] = output_dim_list
        ...
        self.rtmt_fc = nn.Linear(rtmt_hidden_dim, rtmt_output_dim)
    ...
    def forward(self, x_list, h_list):
        [carp_x, rtmo_x, enba_x, rtmt_x, bicm_x, rtms_x] = x_list
        [carp_h, rtmo_h, enba_h, rtmt_h, bicm_h, rtms_h] = h_list
        carp_out, rtmo_out, enba_out, rtmt_out, bicm_out, rtms_out = [], [], [], [], [], []
        ...
        for i in range(carp_x.size(1)):  # loop over the sequence
            ...
            for j in range(0, 24):
                ...
                Ts_hourly = self.sigmoid(self.enba_fc(enba_h[-1]))[:, 0:4]
                enba_hourly_hidden.append(enba_h[-1])

                # RTM thermal
                rtmt_x[:, i, j, 0] = lai_daily
                rtmt_x[:, i, j, 1:5] = Ts_hourly
                rtmt_h[0] = self.rtmt_gru_cells[0](rtmt_x[:, i, j, :].clone(), rtmt_h[0])
                for k in range(1, self.rtmt_n_layers):
                    rtmt_h[k] = self.rtmt_gru_cells[k](rtmt_h[k - 1], rtmt_h[k])
                lst_hourly = self.sigmoid(self.rtmt_fc(rtmt_h[-1]))[:, 0]
                rtmt_hourly_hidden.append(rtmt_h[-1])
                ...
                gpp_hourly = self.sigmoid(self.bicm_fc(bicm_h[-1]))[:, 0]
                fqe_hourly = self.sigmoid(self.bicm_fc(bicm_h[-1]))[:, 1]
                bicm_hourly_hidden.append(bicm_h[-1])

                # RTM sif
                rtms_x[:, i, j, 0] = lai_daily
                rtms_x[:, i, j, 1] = fPAR_hourly
                rtms_x[:, i, j, 3] = fqe_hourly
            ...
            rtmt_out.append(torch.stack(rtmt_hourly_hidden, dim=1))
        ...
        rtmt_out = torch.stack(rtmt_out, dim=0)
        rtmt_out = self.sigmoid(self.rtmt_fc(rtmt_out[-1]))
        ...
        out_list = [carp_out, rtmo_out, enba_out, rtmt_out, bicm_out, rtms_out]

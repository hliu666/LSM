...
class Data:
    ...
    def __init__(self, i, p, data_root):
        ...
        hourly_df = reduce_mem_usage(hourly_df)
        daily_df = reduce_mem_usage(daily_df)
        spectral_df = reduce_mem_usage(spectral_df)
        ...
        self.daily_df = self.daily_df.astype(np.float32)
        self.hourly_df = self.hourly_df.astype(np.float32)

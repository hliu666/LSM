...
class Dataloader:
    def __init__(self, dC, v, p):
        carp_X_train, carp_y_train, carp_X_valid, carp_y_valid, carp_X_test, carp_y_test, self.carp_label_scaler = self.dataLoader(
            dC.daily_df,
            p.lookback_daily,
            p.test_portion,
            v.y_carp_vars,
        ...
        rtmo_X_train, rtmo_y_train, rtmo_X_valid, rtmo_y_valid, rtmo_X_test, rtmo_y_test, self.rtmo_label_scaler = self.dataLoader(
            dC.hourly_df,
            p.lookback_hourly,
            p.test_portion,
            v.y_rtmo_vars,
        ...
        enba_X_train, enba_y_train, enba_X_valid, enba_y_valid, enba_X_test, enba_y_test, self.enba_label_scaler = self.dataLoader(
            dC.hourly_df,
            p.lookback_hourly,
            p.test_portion,
            v.y_enba_vars,
        ...
        bicm_X_train, bicm_y_train, bicm_X_valid, bicm_y_valid, bicm_X_test, bicm_y_test, self.bicm_label_scaler = self.dataLoader(
            dC.hourly_df,
            p.lookback_hourly,
            p.test_portion,
            v.y_bicm_vars,
        ...
        rtms_X_train, rtms_y_train, rtms_X_valid, rtms_y_valid, rtms_X_test, rtms_y_test, self.rtms_label_scaler = self.dataLoader(
            dC.hourly_df,
            p.lookback_hourly,
            p.test_portion,
            v.y_rtms_vars,
        ...
        self.carp_train_loader = DataLoader(carp_train_data, shuffle=True, batch_size=p.batch_size_daily,
                                            collate_fn=lambda x: collate_superv(x, max_len=p.lookback_daily),
                                            drop_last=True)
        self.rtmo_train_loader = DataLoader(rtmo_train_data, shuffle=True, batch_size=p.batch_size_hourly,
                                            collate_fn=lambda x: collate_superv(x, max_len=p.lookback_hourly),
                                            drop_last=True)
        ...
        self.carp_valid_loader = DataLoader(carp_valid_data, shuffle=True, batch_size=p.batch_size_daily,
                                            collate_fn=lambda x: collate_superv(x, max_len=p.lookback_daily),
                                            drop_last=True)
        self.rtmo_valid_loader = DataLoader(rtmo_valid_data, shuffle=True, batch_size=p.batch_size_hourly,
                                            collate_fn=lambda x: collate_superv(x, max_len=p.lookback_hourly),
                                            drop_last=True)
        self.enba_valid_loader = DataLoader(enba_valid_data, shuffle=True, batch_size=p.batch_size_hourly,
                                            collate_fn=lambda x: collate_superv(x, max_len=p.lookback_hourly),
                                            drop_last=True)
        self.bicm_valid_loader = DataLoader(bicm_valid_data, shuffle=True, batch_size=p.batch_size_hourly,
                                            collate_fn=lambda x: collate_superv(x, max_len=p.lookback_hourly),
                                            drop_last=True)
        self.rtms_valid_loader = DataLoader(rtms_valid_data, shuffle=True, batch_size=p.batch_size_hourly,
                                            collate_fn=lambda x: collate_superv(x, max_len=p.lookback_hourly),
                                            drop_last=True)
    ...
    def dataLoader(self, df, lookback, test_portion, y_field, x_field_vars, x_field_pars):

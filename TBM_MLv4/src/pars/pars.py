class Par:
    ...
    def __init__(self, d_model, n_heads, dim_feedforward, n_layers, lookback_periods, batch_size):
        """
        2. define machine learning parameters
        """
        # the embed dimension (required)
        self.carp_d_model = d_model
        self.rtmo_d_model = d_model
        self.enba_d_model = d_model
        self.bicm_d_model = d_model
        self.rtms_d_model = d_model

        # number of heads
        self.carp_n_heads = n_heads
        self.rtmo_n_heads = n_heads
        self.enba_n_heads = n_heads
        self.bicm_n_heads = n_heads
        self.rtms_n_heads = n_heads

        # the dimension of the feedforward network model (default=2048).
        self.carp_dim_feedforward = dim_feedforward
        self.rtmo_dim_feedforward = dim_feedforward
        self.enba_dim_feedforward = dim_feedforward
        self.bicm_dim_feedforward = dim_feedforward
        self.rtms_dim_feedforward = dim_feedforward

        self.carp_n_layers = n_layers
        self.rtmo_n_layers = n_layers
        self.enba_n_layers = n_layers
        self.bicm_n_layers = n_layers
        self.rtms_n_layers = n_layers

        self.lookback_daily = lookback_periods
        self.lookback_hourly = self.lookback_daily * 24

        self.batch_size_daily = batch_size

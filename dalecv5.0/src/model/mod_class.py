...
# ------------------------------------------------------------------------------
    ...
    def dalecv2(self, p):
        ...
        for h in range(0, 24):
            xh = self.x*24+h
            ebal_pars, k_pars = rtm_o(self.dC, xh, lai+sai, hemi_dif_brf)
        ...
        if (lai <= 0.5) and (self.dC.D[self.x*24] > 180): 
            phi_off = 1.0  #only be used in the DBF!
        else:
            phi_off = self.phi_fall(p[13], p[14], p[4])
        ...
        return out, nee_h, lst_h, fPAR_h, refl_mds
    ...
    def mod_list(self, pvals):
        ...
        nee_y, lst_y, fPAR_y, refls = [], [], [], []
        self.x = self.startrun
        for t in xrange(self.endrun-self.startrun):
            mod_list[(t+1)], nee_d, lst_d, fPAR_d, refl_d = self.dalecv2(mod_list[t])
            nee_y  += nee_d
            lst_y  += lst_d
            fPAR_y += fPAR_d
            if len(refl_d) > 1:
        ...
        self.x -= self.endrun
        return mod_list, np.array(nee_y), np.array(lst_y), np.array(fPAR_y), np.array(refls)

...
def PhotoSynth(meteo, Photo_Pars):
    ...
    if Type == "C4":
        ...
        deltaHa     = 65330  #Unit is  [J K^-1]
        deltaS      = 485    #Unit is [J mol^-1 K^-1]
        deltaHd     = 149250 #Unit is [J mol^-1]
        ...
        deltaHa     = 46390  #Unit is  [J K^-1]
        deltaS      = 490    #Unit is [J mol^-1 K^-1]
        deltaHd     = 150650 #Unit is [J mol^-1]
        fTv         = temperature_functionC3(Tref,R,T,deltaHa);
        fHTv        = high_temp_inhibtionC3(Tref,R,T,deltaS,deltaHd);
        f_Rd        = fTv * fHTv
        
        # temperature correction for Kc
        deltaHa     = 79430		# Unit is  [J K^-1]
        fTv         = temperature_functionC3(Tref,R,T,deltaHa)
        f_Kc        = fTv
        
        # temperature correction for Ko
        deltaHa     = 36380     # Unit is  [J K^-1]
        fTv         = temperature_functionC3(Tref,R,T,deltaHa)
        f_Ko        = fTv
        
        # temperature correction for Gamma_star
        deltaHa     = 37830     # Unit is  [J K^-1]
        fTv         = temperature_functionC3(Tref,R,T,deltaHa)
        f_Gamma_star= fTv
